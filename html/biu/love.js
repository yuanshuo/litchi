...
function Next() {
    ...
    block.style.left = block_left + 70 * offset_pitn["block" + index][0] + "px";
    block.style.top = block_top - 70 * offset_pitn["block" + index][1] + "px";
    for (let i = 0; i < block.children.length; i++) {
        // block.children[1].innerText = index;    //编号便于调试
        block.children[i].style.left = blk_pitn["block" + index][i][0] * -70 + "px";
        /* -40 是因为逻辑坐标和浏览器的x，y轴方向不一样*/
        block.children[i].style.top = blk_pitn["block" + index][i][1] * -70 + "px";
    }
